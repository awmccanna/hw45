...
public class ManagerRun implements Runnable
{
	...
	private int waitTime, block1, block2, block3;
	...
	public void run()
	{
		while(jobs.keepGoing())
		{
			...
			if(jobs.numJobs() > block1 && pool.numberThreadsRunning() == 5)//More than 10 jobs, but only 5 threads
			{
			...
			else if(jobs.numJobs() > block2 && pool.numberThreadsRunning() == 10)//More than 20 jobs, only 10 threads
			{
